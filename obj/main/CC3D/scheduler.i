...
_Bool queueContains(cfTask_t *task)
{
...
_Bool queueRemove(cfTask_t *task)
{
    for (int ii = 0; ii < taskQueueSize; ++ii) {
        if (taskQueueArray[ii] == task) {
...
void setTaskEnabled(cfTaskId_e taskId, _Bool enabled)
{
    if (taskId == TASK_SELF || taskId < TASK_COUNT) {
        cfTask_t *task = taskId == TASK_SELF ? currentTask : &cfTasks[taskId];
...
void scheduler(void)
{
    ...
    const _Bool outsideRealtimeGuardInterval = (timeToNextRealtimeTask > 0);
