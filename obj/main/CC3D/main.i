...
typedef struct pwmOutputConfiguration_s {
    ...
    uint8_t motorCount;
    uint8_t outputCount;
    pwmPortConfiguration_t portConfigurations[12];
...
# 1 "./src/main/drivers/bus_bst.h" 1
# 18 "./src/main/drivers/bus_bst.h"
...
_Bool bstWriteBusy(void);
_Bool bstMasterWrite(uint8_t* data);
_Bool bstSlaveRead(uint8_t* buf);
_Bool bstSlaveWrite(uint8_t* data);
...
_Bool sensorsAutodetect(sensorAlignmentConfig_t *sensorAlignmentConfig, uint8_t accHardwareToUse, uint8_t magHardwareToUse, uint8_t baroHardwareToUse, int16_t magDeclinationFromConfig, uint8_t gyroLpf, uint8_t gyroSyncDenominator);
...
# 285 "./src/main/main.c"
    ...
    if (masterConfig.use_oneshot42) {
        pwm_params.useOneshot42 = masterConfig.use_oneshot42 ? 1 : 0;
        masterConfig.use_multiShot = 0;
    } else {
        pwm_params.useMultiShot = masterConfig.use_multiShot ? 1 : 0;
    }
    ...
    pwm_params.useBuzzerP6 = masterConfig.use_buzzer_p6 ? 1 : 0;
...
# 456 "./src/main/main.c"
    ...
    for (i = 0; i < 10; i++) {
        ...
        delay(25);
        if (!(getPreferedBeeperOffMask() & (1 << (BEEPER_SYSTEM_INIT - 1)))) systemBeep(1);
        delay(25);
        systemBeep(0);
    }
    digitalHi(led_config[0].gpio, led_config[0].pin);
...
# 584 "./src/main/main.c"
    ...
    setTargetPidLooptime(masterConfig.pid_process_denom);
