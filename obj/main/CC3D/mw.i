...
extern _Bool antiWindupProtection;

uint16_t filteredCycleTime;
static _Bool isRXDataNew;
static _Bool armingCalibrationWasInitialised;
...
void scaleRcCommandToFpvCamAngle(void) {
    ...
    if (lastFpvCamAngleDegrees != masterConfig.rxConfig.fpvCamAngleDegrees){
        ...
        cosFactor = cos_approx(masterConfig.rxConfig.fpvCamAngleDegrees * (3.14159265358979323846f / 180.0f));
        sinFactor = sin_approx(masterConfig.rxConfig.fpvCamAngleDegrees * (3.14159265358979323846f / 180.0f));
    }
...
void annexCode(void)
{
    ...
    if (feature(FEATURE_3D) && ((1 << (BOX3DDISABLESWITCH)) & rcModeActivationMask) && !failsafeIsActive()) {
        fix12_t throttleScaler = qConstruct(rcCommand[THROTTLE] - 1000, 1000);
        rcCommand[THROTTLE] = masterConfig.rxConfig.midrc + qMultiply(throttleScaler, 2000 - masterConfig.rxConfig.midrc);
    }
    ...
    if (masterConfig.rxConfig.fpvCamAngleDegrees && !(flightModeFlags & (HEADFREE_MODE))) {
    ...
    if ((armingFlags & (ARMED))) {
        ...
        if (((1 << (BOXARM)) & rcModeActivationMask) == 0 || armingCalibrationWasInitialised) {
...
void mwDisarm(void)
{
    armingCalibrationWasInitialised = 0;
...
void mwArm(void)
{
    static _Bool firstArmingCalibrationWasCompleted;
    ...
    if (masterConfig.gyro_cal_on_first_arm && !firstArmingCalibrationWasCompleted) {
        ...
        armingCalibrationWasInitialised = 1;
        firstArmingCalibrationWasCompleted = 1;
    }

    if (!isGyroCalibrationComplete()) return;
    ...
    if (throttleStatus == THROTTLE_LOW) {
        if (((1 << (BOXAIRMODE)) & rcModeActivationMask) && !failsafeIsActive() && (armingFlags & (ARMED))) {
            if (rollPitchStatus == CENTERED) {
                antiWindupProtection = 1;
            } else {
                antiWindupProtection = 0;
            }
        } else {
...
# 707 "./src/main/mw.c"
        ...
        if (masterConfig.throttle_correction_value && ((flightModeFlags & (ANGLE_MODE)) || (flightModeFlags & (HORIZON_MODE)))) {
...
_Bool shouldUpdateMotorsAfterPIDLoop(void) {
...
void taskMainPidLoopCheck(void) {
    ...
    static _Bool runTaskMainSubprocesses;
    ...
    while (1) {
        if (gyroSyncCheckUpdate() || ((currentDeltaTime + (micros() - previousTime)) >= (targetLooptime + 80))) {
            static uint8_t pidUpdateCountdown;

            if (runTaskMainSubprocesses) {
                if (!shouldUpdateMotorsAfterPIDLoop()) taskMotorUpdate();
                subTasksMainPidLoop();
                runTaskMainSubprocesses = 0;
            }
