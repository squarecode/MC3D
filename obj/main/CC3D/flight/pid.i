...
static _Bool lowThrottlePidReduction;
...
float calculateExpoPlus(int axis, rxConfig_t *rxConfig) {
    ...
    if (axis == YAW && !rxConfig->superExpoYawMode) {
        ...
        propFactor = 1.0f - ((superExpoFactor / 100.0f) * (((rcCommand[axis]) > 0 ? (rcCommand[axis]) : -(rcCommand[axis])) / 500.0f));
    }
...
void pidResetErrorGyroState(uint8_t resetOption)
{
    ...
    if (resetOption == RESET_ITERM_AND_REDUCE_PID) {
        lowThrottlePidReduction = 1;
    } else {
        lowThrottlePidReduction = 0;
    }
}
        ...
        uint16_t max_angle_inclination, rollAndPitchTrims_t *angleTrim, rxConfig_t *rxConfig)
{
    ...
    static float lastRate[3][7];
    float delta;
    int axis;
    float horizonLevelStrength = 1;

    float tpaFactor = PIDweight[0] / 100.0f;
    ...
    if ((flightModeFlags & (HORIZON_MODE))) {
        ...
        const int32_t stickPosAil = ((getRcStickDeflection(FD_ROLL, rxConfig->midrc)) > 0 ? (getRcStickDeflection(FD_ROLL, rxConfig->midrc)) : -(getRcStickDeflection(FD_ROLL, rxConfig->midrc)));
        const int32_t stickPosEle = ((getRcStickDeflection(FD_PITCH, rxConfig->midrc)) > 0 ? (getRcStickDeflection(FD_PITCH, rxConfig->midrc)) : -(getRcStickDeflection(FD_PITCH, rxConfig->midrc)));
        const int32_t mostDeflectedPos = ((stickPosAil) > (stickPosEle) ? (stickPosAil) : (stickPosEle));
    ...
    for (axis = 0; axis < 3; axis++) {
        ...
        if ((((1 << (BOXSUPEREXPO)) & rcModeActivationMask) && axis != YAW) || (axis == YAW && rxConfig->superExpoYawMode == SUPEREXPO_YAW_ALWAYS)) {
        ...
        if ((pidProfile->rollPitchItermResetAlways || ((1 << (BOXSUPEREXPO)) & rcModeActivationMask)) && axis != YAW) {
            if (((gyroRate / 4.1f) > 0 ? (gyroRate / 4.1f) : -(gyroRate / 4.1f)) >= pidProfile->rollPitchItermResetRate) errorGyroIf[axis] = constrainf(errorGyroIf[axis], -20, 20);
        }

        if (axis == YAW) {
            if (((gyroRate / 4.1f) > 0 ? (gyroRate / 4.1f) : -(gyroRate / 4.1f)) >= pidProfile->yawItermResetRate) errorGyroIf[axis] = constrainf(errorGyroIf[axis], -10, 10);
        }
        ...
        rollAndPitchTrims_t *angleTrim, rxConfig_t *rxConfig)
{
    ...
    for (axis = 0; axis < 3; axis++) {
        ...
        if ((pidProfile->rollPitchItermResetAlways || ((1 << (BOXSUPEREXPO)) & rcModeActivationMask)) && axis != YAW) {
            if (((gyroRate *10 / 41) > 0 ? (gyroRate *10 / 41) : -(gyroRate *10 / 41)) >= pidProfile->rollPitchItermResetRate) errorGyroI[axis] = constrain(errorGyroI[axis], -20, 20);
        }

        if (axis == YAW) {
            if (((gyroRate * 10 / 41) > 0 ? (gyroRate * 10 / 41) : -(gyroRate * 10 / 41)) >= pidProfile->yawItermResetRate) errorGyroI[axis] = constrain(errorGyroI[axis], -10, 10);
        }

        if (antiWindupProtection || motorLimitReached) {
            errorGyroI[axis] = constrain(errorGyroI[axis], -errorGyroILimit[axis], errorGyroILimit[axis]);
        } else {
            errorGyroILimit[axis] = ((errorGyroI[axis]) > 0 ? (errorGyroI[axis]) : -(errorGyroI[axis]));
        }
