...
void mixTable(void)
{
    ...
    if (batteryConfig->vbatPidCompensation) vbatCompensationFactor = calculateVbatPidCompensation();
    ...
    for (i = 0; i < motorCount; i++) {
        ...
        if (batteryConfig->vbatPidCompensation) rollPitchYawMix[i] = qMultiply(vbatCompensationFactor, rollPitchYawMix[i]);
    ...
    for (i = 0; i < motorCount; i++) {
        ...
        if (feature(FEATURE_MOTOR_STOP) && (armingFlags & (ARMED)) && !feature(FEATURE_3D) && !((1 << (BOXAIRMODE)) & rcModeActivationMask)) {
