...
static biquad_t accFilterState[3];
static _Bool accFilterInitialised = 0;
    ...
    if (accLpfCutHz) {
        if (!accFilterInitialised) {
            if (accTargetLooptime) {
                ...
                accFilterInitialised = 1;
            }
        }

        if (accFilterInitialised) {
            for (axis = 0; axis < 3; axis++) accSmooth[axis] = lrintf(applyBiQuadFilter((float) accSmooth[axis], &accFilterState[axis]));
        }
    }
