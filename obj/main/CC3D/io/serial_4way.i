...
# 47 "./src/main/io/serial_4way.c" 2
...
inline _Bool isMcuConnected(void)
{
    return (DeviceInfo.bytes[0] > 0);
}

inline _Bool isEscHi(uint8_t selEsc)
{
    return (digitalIn(escHardware[selEsc].gpio, escHardware[selEsc].pin) != Bit_RESET);
}
inline _Bool isEscLo(uint8_t selEsc)
{
    return (digitalIn(escHardware[selEsc].gpio, escHardware[selEsc].pin) == Bit_RESET);
}
    ...
    for (volatile uint8_t i = 0; i < pwmOutputConfiguration->outputCount; i++) {
        if ((pwmOutputConfiguration->portConfigurations[i].flags & PWM_PF_MOTOR) == PWM_PF_MOTOR) {
            if(motor[pwmOutputConfiguration->portConfigurations[i].index] > 0) {
                ...
                escHardware[escCount].gpio_config_INPUT.speed = Speed_2MHz;
...
void DeInitialize4WayInterface(void)
{
    while (escCount > 0) {
        escCount--;
        escHardware[escCount].gpio_config_OUTPUT.mode = Mode_AF_PP;
...
static uint8_t Connect(uint8_32_u *pDeviceInfo)
{
    for (uint8_t I = 0; I < 3; ++I) {
        ...
        if (Stk_ConnectEx(pDeviceInfo) && ((pDeviceInfo->words[0] == 0x9307) || (pDeviceInfo->words[0] == 0x930A) || (pDeviceInfo->words[0] == 0x930F) || (pDeviceInfo->words[0] == 0x940B))) {
        ...
        if (ACK_OUT == 0x00)
        {
            ...
            switch(CMD) {
                ...
                case 0x33:
                {
                    ...
                    O_PARAM_LEN = 2;
                    Dummy.bytes[0] = (uint8_t) ((uint16_t) ((14 * 1000) + ((uint8_t) 4 * 100) + (uint8_t) 04) / 100);
                    Dummy.bytes[1] = (uint8_t) ((uint16_t) ((14 * 1000) + ((uint8_t) 4 * 100) + (uint8_t) 04) % 100);
                    break;
                }
                case 0x34:
                {
