...
void setGyroSamplingSpeed(uint16_t looptime) {
    ...
    if (looptime != targetLooptime || looptime == 0) {
        if (looptime == 0) looptime = targetLooptime;
# 145 "./src/main/io/serial_msp.c"
        if (looptime < 1000) {
...
const char * const flightControllerIdentifier = "BTFL";
...
static uint32_t packFlightModeFlags(void)
{
    ...
    tmp = ((flightModeFlags & (ANGLE_MODE)) == 0 ? 0 : 1) << BOXANGLE |
        ...
        (((1 << (BOXSUPEREXPO)) & rcModeActivationMask) == 0 ? 0 : 1) << BOXSUPEREXPO;
...
static _Bool processInCommand(void)
{
    ...
    case 60:
        ...
        if (oldPid != currentProfile->pidProfile.pidController) setGyroSamplingSpeed(0);
        break;
    case 202:
        for (i = 0; i < PID_ITEM_COUNT; i++) {
    ...
    case 81:
        ...
        break;
# 1531 "./src/main/io/serial_msp.c"
    case 72:
        flashfsEraseCompletely();
        break;
# 1575 "./src/main/io/serial_msp.c"
    case 37:
        featureClearAll();
        featureSet(read32());
        break;
