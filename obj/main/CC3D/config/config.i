...
# 74 "./src/main/config/config.c" 2

#define BRUSHED_MOTORS_PWM_RATE 16000
#define BRUSHLESS_MOTORS_PWM_RATE 400

void useRcControlsConfig(modeActivationCondition_t *modeActivationConditions, escAndServoConfig_t *escAndServoConfigToUse, pidProfile_t *pidProfileToUse);
# 91 "./src/main/config/config.c"
#define FLASH_PAGE_SIZE ((uint16_t)0x400)
# 110 "./src/main/config/config.c"
#define FLASH_PAGE_COUNT ((FLASH_SIZE * 0x400) / FLASH_PAGE_SIZE)
# 122 "./src/main/config/config.c"
...
static void resetPidProfile(pidProfile_t *pidProfile)
{
    ...
    pidProfile->dterm_lpf_hz = 70.0f;

    pidProfile->H_sensitivity = 75;
# 200 "./src/main/config/config.c"
}
# 215 "./src/main/config/config.c"
...
# 397 "./src/main/config/config.c"
    ...
    masterConfig.gyro_cal_on_first_arm = 0;
...
# 495 "./src/main/config/config.c"
 ...
 for (rI = 0; rI<3; rI++) {
  resetControlRateConfig(&masterConfig.profile[0].controlRateProfile[rI]);
    ...
    masterConfig.throttle_correction_value = 0;
    masterConfig.throttle_correction_angle = 800;
    ...
    for (i = 0; i < 8; i++) {
        masterConfig.servoConf[i].min = 1000;
        masterConfig.servoConf[i].max = 2000;
        masterConfig.servoConf[i].middle = 1500;
        masterConfig.servoConf[i].rate = 100;
        masterConfig.servoConf[i].angleAtMin = 90;
        masterConfig.servoConf[i].angleAtMax = 90;
        masterConfig.servoConf[i].forwardFromChannel = (uint8_t)0xFF;
    }
...
{
    const master_t *temp = (const master_t *) (0x08000000 + (uint32_t)((((uint16_t)0x400) * ((128 * 0x400) / ((uint16_t)0x400))) - 0x800));
    uint8_t checksum = 0;
...
void readEEPROM(void)
{
    ...
    memcpy(&masterConfig, (char *) (0x08000000 + (uint32_t)((((uint16_t)0x400) * ((128 * 0x400) / ((uint16_t)0x400))) - 0x800)), sizeof(master_t));

    if (masterConfig.current_profile_index > 2 - 1)
        masterConfig.current_profile_index = 0;
...
void writeEEPROM(void)
{
    ...
    ((void)sizeof(char[1 - 2*!!(sizeof(master_t) > 0x800)]));
    ...
    while (attemptsRemaining--) {
        ...
        for (wordOffset = 0; wordOffset < sizeof(master_t); wordOffset += 4) {
            if (wordOffset % ((uint16_t)0x400) == 0) {
                status = FLASH_ErasePage((0x08000000 + (uint32_t)((((uint16_t)0x400) * ((128 * 0x400) / ((uint16_t)0x400))) - 0x800)) + wordOffset);
                if (status != FLASH_COMPLETE) {
                    break;
                }
            }

            status = FLASH_ProgramWord((0x08000000 + (uint32_t)((((uint16_t)0x400) * ((128 * 0x400) / ((uint16_t)0x400))) - 0x800)) + wordOffset,
