...
{
    uint8_t i;
    uint16_t minThrottle = (feature(FEATURE_3D && ((1 << (BOX3DDISABLESWITCH)) & rcModeActivationMask)) ? 1000 : escAndServoConfig->minthrottle);
    ...
    for (i = 0; i < 12; i++) {
        ...
        lookupThrottleRC[i] = minThrottle + (int32_t) (escAndServoConfig->maxthrottle - minThrottle) * lookupThrottleRC[i] / 1000;
    }
}
